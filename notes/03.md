...
And the time taken is 250ms, which is a huge improvement. We will call it an end here for matrix multiplication.
...
Not the time reduced to 40ms! This is great achievements. But please note that to make local memory efficient, you must first do memory coalescing, which, albeit didn't yield much improvement, is still important.

Now we call it an end for matrix multiplication. There are only two more trick left, not included in this case.
